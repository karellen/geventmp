...
if __name__ == "__main__":
    ...
    if sys.argv[1] == "install":
        ...
        deploy_gevents = environ["DEPLOY_GEVENTS"].split()
        ...
        if is_production:
            ...
            if (environ["TRAVIS_OS_NAME"] in deploy_oses and
                    ...
                    environ["TRAVIS_BRANCH"] in deploy_branches and
                    environ.get("GEVENT_VERSION") in deploy_gevents):
        ...
        cmd_args = (["pyb"] + pyb_args + (["-P", "gevent_version=%s" % environ["GEVENT_VERSION"]]
                                          if environ.get("GEVENT_VERSION") else []))

        print("Will run build with the following args: %r" % cmd_args)

        sys.stdout.flush()

        subprocess.check_call(cmd_args, env=environ, cwd=project_dir)
